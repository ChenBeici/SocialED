...
from datasets import Event2012_Dataset, Event2018_Dataset, MAVEN_Dataset, Arabic_Dataset
...
class args_define():
    ...
    parser.add_argument('--data_path', default='./clkd/English',
    ...
    parser.add_argument('--Tmodel_path',
                        default='./clkd/English/Tmodel/',#'803_hash_static-8-English/0mode/embeddings_0227165510-0-English-nomap',
                        type=str,
    ...
    parser.add_argument('--data_path1', default='./clkd/English',
                        type=str, help="Path of features, labels and edges")
    parser.add_argument('--data_path2', default='./clkd/French',
                        type=str, help="Path of features, labels and edges")
    ...
    parser.add_argument('--embpath',type=str,default='./clkd/dictrans/fr-en-for.npy')    
    parser.add_argument('--wordpath',type=str,default='./clkd/dictrans/wordsFrench.txt')
    ...
    def forward(self, blocks, args, trans=False, src=None, tgt=None):
        ...
        if trans:
            ...
            if args.mode == 2 and args.add_mapping:
                ...
                W = torch.from_numpy(
                    torch.load('./clkd/LinearTranWeight/spacy_{}_{}/best_mapping.pth'.format(src, tgt))).float()
...
class CLKD:
    ...
    def detection(self):
        ...
        if self.args.mutual:
            ...
            predictions.append(extract_features1)
            ground_truths.append(labels1)
            predictions.append(extract_features2)
            ground_truths.append(labels2)
        else:
            if self.args.is_incremental:
            ...
            #else:
            #    self.model = infer(self.train_i, 0, self.data_split, metrics, self.embedding_save_path, loss_fn, self.model)
            ...
            self.detection_path = self.embedding_save_path + '/detection_split/'
    ...
    def evaluate(self, predictions, ground_truths):
        ...
        print(f"Adjusted Rand Index (ARI): {ars}")
...
class Preprocessor:
    ...
    def getlinear_transform_features(self, features, src, tgt):
        W = torch.load("./clkd/LinearTranWeight/spacy_{}_{}/best_mapping.pth".format(src, tgt))
    ...
    def construct_graph(self):
        # create save path
        if args.is_static:
            save_path = "./clkd/hash_static-{}-{}/".format(str(args.days), args.graph_lang)
        else:
            save_path = "./clkd/{}/".format(args.graph_lang)
        ...
        nf = None
        # load features
        f = np.load('./clkd/features/features_69612_0709_spacy_lg_zero_multiclasses_filtered_{}.npy'.format(args.graph_lang))
        nonleafilename = "./clkd/features/features_69612_0709_spacy_lg_zero_multiclasses_filtered_{}_{}.npy".format(args.graph_lang,args.tgtlang)
